...
class Sudoku_algoritma(context: Context) {
    ...
    fun addNumber(i: Int, j: Int, value: Int) {
        Log.i("Bilgi", "Ekle $value")
        if (i != -1 && j != -1) {
            this.solution?.get(i)?.set(j, value)
        } else {
            Log.i("SayıEkle", "Unable to add number as no box is selected")
        }
    }
    fun removeNumber(i: Int, j: Int) {
        Log.i("Sil", "Removing number")
        if (i == -1 || j == -1) {
            Log.i("Sil","Removing number failed as no box is selected")
        } else if (this.solution?.get(i)?.get(j) == -1) {
            Log.i("Sil","Removing number failed as selected box is a constant in the problem")
        } else {
            this.solution?.get(i)?.set(j, -1)
            Log.i("Sil", "Removed number at ($i, $j)")
        }
    }
}